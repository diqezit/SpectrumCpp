...
namespace Spectrum {
    ...
    private:
        ...
        [[nodiscard]] bool IsPeakArraySizeCorrect(size_t size) const;

        // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        // Peak Update Logic
        // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=

        void UpdatePeaks(const SpectrumData& spectrum, float deltaTime);
        void UpdateSinglePeak(size_t index, float value, float deltaTime);

        void SetNewPeak(size_t index, float value);
        void DecrementPeakHoldTimer(size_t index, float deltaTime);
        void ApplyPeakFall(size_t index, float deltaTime);

        [[nodiscard]] bool ShouldSetNewPeak(size_t index, float value) const;
        [[nodiscard]] bool IsPeakHolding(size_t index) const;
        ...
        [[nodiscard]] bool IsPeakValueVisible(float peakValue) const;
