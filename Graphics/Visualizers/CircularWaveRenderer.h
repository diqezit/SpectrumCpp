...
namespace Spectrum {
    ...
    protected:
        ...
        void DoRender(BLContext& ctx, const SpectrumData& spectrum) override {
            const int rings = std::min(int(spectrum.size()), m_settings.maxRings);
            if (rings <= 0) return;

            const Point center = GetViewportCenter();
            const float maxRadius = GetMaxRadius();
            const float step = (maxRadius - kCenterRadius) / float(rings);

            for (int i = rings - 1; i >= 0; --i) {
                const float mag = RenderUtils::SegmentAverage(spectrum, size_t(rings), size_t(i));
                if (mag < kMinMagnitude) continue;

                const float radius = kCenterRadius
                    + float(i) * step
                    + std::sin(m_waveTime + float(i) * 0.1f + m_angle) * mag * step;
