...
namespace Spectrum::Draw {
    namespace detail {
        ...
        struct ScopedStroke {
            ...
            ScopedStroke(BLContext& c, float w, bool caps, bool join) : ctx(c) {
                ...
                if (caps) ctx.set_stroke_caps(BL_STROKE_CAP_ROUND);
                if (join) ctx.set_stroke_join(BL_STROKE_JOIN_ROUND);
            }
    ...
    inline void FillSector(BLContext& ctx, const Point& c, float r,
    ...
    inline void StrokeArc(BLContext& ctx, const Point& c, float r,
    ...
    inline void Glow(BLContext& ctx, const Point& c, float r, const Color& color,
        float intensity = 1.0f, int layers = 5) {
        for (int i = layers; i > 0; --i)
            FillCircle(ctx, c, r + i * 2.f,
                Color(color.r, color.g, color.b, (1.f - float(i) / float(layers)) * 0.2f * intensity * color.a));
    }
    ...
    inline const BLFont* UiFont(float size) {
        ...
        if (last < 0.f)
            for (auto* p : { "C:\\Windows\\Fonts\\seguisym.ttf",
                             "C:\\Windows\\Fonts\\segoeui.ttf",
                             "C:\\Windows\\Fonts\\arial.ttf" })
                if (face.create_from_file(p) == BL_SUCCESS) break;
        if (last != size) { font.create_from_face(face, size); last = size; }
        return &font;
    }

    inline void FillTextCentered(BLContext& ctx, std::string_view utf8,
        ...
        ctx.fill_utf8_text(
            BLPoint(rect.x + (rect.width - float(tm.bounding_box.x1 - tm.bounding_box.x0)) * 0.5f
                - float(tm.bounding_box.x0),
                rect.y + (rect.height + fm.ascent - fm.descent) * 0.5f),
            font, utf8.data(), utf8.size(), ToBL(color));
    }
