...
namespace Spectrum {
    ...
    ) const
    {
        ...
        const float sanitizedIntensity = Sanitize::NormalizedFloat(intensity);
        constexpr int kMinLayers = 3;
        constexpr int kMaxLayers = 10;
        constexpr int kDefaultLayers = 5;
        const int layers = Sanitize::ClampValue(kDefaultLayers, kMinLayers, kMaxLayers);
        ...
        for (int i = 0; i < layers; ++i) {
            ...
            HRESULT hr = m_renderTarget->CreateGradientStopCollection(
                stops, 2, stopCollection.GetAddressOf()
            );
            ...
            hr = m_renderTarget->CreateRadialGradientBrush(
                props, stopCollection.Get(), gradientBrush.GetAddressOf()
            );
            if (FAILED(hr)) continue;
    ...
    void EffectsRenderer::BeginOpacityLayer(float opacity) const
    {
        ...
        ScopedOpacityLayer layer(m_renderTarget, opacity);
    }
