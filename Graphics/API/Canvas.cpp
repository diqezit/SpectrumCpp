...
namespace Spectrum {
    ...
    {
        if (m_primitiveRenderer) {
            m_primitiveRenderer->DrawEllipse(
                center,
                radiusX,
                radiusY,
                paint.color,
    ...
    {
        if (m_primitiveRenderer) {
            m_primitiveRenderer->DrawPolygon(
                points,
                paint.color,
                paint.isFilled,
                paint.strokeWidth
    ...
    {
        if (m_primitiveRenderer) {
            m_primitiveRenderer->DrawSector(
                center,
                radius,
                startAngle,
                sweepAngle,
    ...
    {
        if (m_primitiveRenderer) {
            m_primitiveRenderer->DrawRegularPolygon(
                center,
                radius,
                sides,
                rotation,
    ...
    {
        if (m_primitiveRenderer) {
            m_primitiveRenderer->DrawCircleBatch(
                centers,
                radius,
                paint.color,
                paint.isFilled
    ...
    {
        if (m_primitiveRenderer) {
            m_primitiveRenderer->DrawRectangleBatch(
                rects,
                paint.color,
                paint.isFilled
            );
    ...
    void Canvas::DrawGradientRectangle(
        const Rect& rect,
    ...
    ) const
    {
        if (m_gradientRenderer) {
            m_gradientRenderer->DrawGradientRectangle(
                rect,
                stops,
                horizontal
            );
    ...
    void Canvas::DrawRadialGradient(
        const Point& center,
        float radius,
        const std::vector<D2D1_GRADIENT_STOP>& stops
    ) const
    {
        if (m_gradientRenderer) {
            m_gradientRenderer->DrawRadialGradient(
                center,
                radius,
                stops
            );
    ...
    void Canvas::DrawGradientCircle(
        const Point& center,
        float radius,
        const std::vector<D2D1_GRADIENT_STOP>& stops,
        bool filled
    ...
    {
        if (m_gradientRenderer) {
            m_gradientRenderer->DrawGradientCircle(
                center,
                radius,
                stops,
                filled
    ...
    {
        if (m_gradientRenderer) {
            m_gradientRenderer->DrawGradientPath(
                points,
                stops,
                strokeWidth
            );
    ...
    void Canvas::DrawAngularGradient(
        const Point& center,
        float radius,
        float startAngle,
        float endAngle,
    ...
    {
        if (m_gradientRenderer) {
            m_gradientRenderer->DrawAngularGradient(
                center,
                radius,
                startAngle,
                endAngle,
    ...
    void Canvas::DrawVerticalGradientBar(
        const Rect& rect,
        const std::vector<D2D1_GRADIENT_STOP>& stops,
        float cornerRadius
    ) const
    ...
    ) const
    {
        if (m_effectsRenderer) {
            m_effectsRenderer->DrawWithShadow(
                drawCallback,
                offset,
                blur,
                shadowColor
    ...
    void Canvas::DrawTextWithOutline(
        const std::wstring& text,
        const Point& position,
    ...
    {
        if (m_textRenderer) {
            m_textRenderer->DrawTextWithOutline(
                text,
                position,
                fillColor,
                outlineColor,
    ...
    {
        if (m_textRenderer) {
            m_textRenderer->DrawTextRotated(
                text,
                position,
                angleDegrees,
                color,
    ...
    ) const
    {
        if (m_spectrumRenderer) {
            m_spectrumRenderer->DrawSpectrumBars(
                spectrum,
                bounds,
                style,
                color
    ...
    {
        if (m_spectrumRenderer) {
            m_spectrumRenderer->DrawWaveform(
                spectrum,
                bounds,
                color,
                strokeWidth,
