...
namespace Spectrum
{
    ...
    [[nodiscard]] bool RendererManager::Initialize()
    {
        ...
        if (!ValidateWindowManager()) {
            LOG_ERROR("RendererManager: WindowManager validation failed");
            return false;
        }
    ...
    ) const
    {
        ...
        return ValidateActivationContext(outContext);
    }

    bool RendererManager::TryActivateRenderer(ActivationContext& context) const noexcept
    {
        if (!ValidateActivationContext(context)) {
            return false;
        }

        try {
            // Activate renderer with dimensions
    ...
    [[nodiscard]] bool RendererManager::ValidateRenderer(IRenderer* renderer) const noexcept
    {
        return renderer != nullptr;
    }

    [[nodiscard]] bool RendererManager::ValidateWindowManager() const noexcept
    {
        if (!m_windowManager) {
            LOG_ERROR("RendererManager: WindowManager is null");
            return false;
    ...
    [[nodiscard]] bool RendererManager::ValidateRenderEngine() const noexcept
    {
        if (!m_windowManager) {
            return false;
        }
        ...
        if (!engine) {
            LOG_ERROR("RendererManager: RenderEngine is null");
            return false;
        }
    ...
    [[nodiscard]] bool RendererManager::ValidateActivationContext(
        const ActivationContext& context
    ) const noexcept
    {
        if (!context.renderer) {
            LOG_ERROR("RendererManager: Activation context has null renderer");
            return false;
        }
    ...
    ) const
    {
        ...
        (void)reason;  // Suppress warning
        LOG_ERROR("RendererManager: Failed to activate renderer");
    }
