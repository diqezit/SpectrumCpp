...
namespace Spectrum {
    ...
    void LedPanelRenderer::RenderInactiveLeds(GraphicsContext& context) const
    {
        ...
        context.DrawCircleBatch(m_allLedPositions, kLedRadius, color, true);
    }

    void LedPanelRenderer::RenderActiveLeds(GraphicsContext& context) const
    {
        for (int col = 0; col < m_grid.columns; ++col) {
            ...
            for (int row = 0; row < activeLeds; ++row) {
                ...
                context.DrawCircle(m_allLedPositions[ledIndex], kLedRadius, ledColor, true);
            }
        }
    }

    void LedPanelRenderer::RenderPeakLeds(GraphicsContext& context) const
    {
        for (int col = 0; col < m_grid.columns; ++col) {
            ...
            if (peakRow >= 0 && peakRow < m_grid.rows) {
                const size_t ledIndex = static_cast<size_t>(col) * m_grid.rows + peakRow;
