...
namespace Spectrum {
    ...
    ) const
    {
        ...
        context.DrawSpectrumBars(spectrum, bounds, barStyle, kSolidBarColor);
    }
    ...
    ) const
    {
        ...
        if (peakCornerRadius > 0.0f) {
            for (const auto& rect : peakRects) {
                context.DrawRoundedRectangle(rect, peakCornerRadius, kPeakColor, true);
            }
        }
        else {
            context.DrawRectangleBatch(peakRects, kPeakColor, true);
        }
    }
