...
namespace Spectrum {
    ...
    )
    {
        ...
        for (size_t i = 0; i < barCount; ++i) {
            ...
            RenderBar(context, rect, magnitude);
        }
    }
    ...
    {
        auto drawCall = [this, &context, &rect, &color]() {
    ...
    ) const
    {
        if (m_settings.cornerRadius > 0.0f) {
            context.DrawRoundedRectangle(rect, m_settings.cornerRadius, color, true);
        }
        else {
    ...
    ) const
    {
        ...
        context.DrawRectangle(
            highlightRect,
            Color(1.0f, 1.0f, 1.0f, kHighlightAlpha * magnitude),
            true
        );
    }
